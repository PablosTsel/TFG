#!/usr/bin/env python3
# Simple Concatenation Damage Classification Model
# This version uses simple concatenation instead of the attention fusion mechanism
# to demonstrate the importance of intelligent feature fusion
...
# Simple concatenation fusion module (alternative to AttentionFusion)
class SimpleConcatFusion(nn.Module):
    """Simple feature fusion by concatenation and convolution"""
    def __init__(self, in_channels):
        super(SimpleConcatFusion, self).__init__()
        
        # Simple convolution layers after concatenation
...
class SimpleConcatDamageClassifier(nn.Module):
    """
    Damage classifier that uses a simple concatenation approach 
    instead of the more sophisticated attention fusion
    """
    def __init__(self, in_channels=3, out_channels=5):
        super(SimpleConcatDamageClassifier, self).__init__()
        ...
        # Simple concat fusion module instead of attention fusion
        self.simple_fusion = SimpleConcatFusion(in_channels=out_channels)
    ...
    def forward(self, pre_img, post_img):
        ...
        # Fuse features with simple concatenation
        fused_features = self.simple_fusion(pre_features, post_features)
...
def visualize_simpleconcat_predictions(model, dataset, device, num_samples=4, save_path=None):
    """
...
def main():
    """
    Main function to run the simple concatenation damage classification training pipeline.
    """
    ...
    # Save configuration
    config = {
        "experiment": "Simple Concatenation (No Attention)",
        "timestamp": timestamp,
    ...
    # Using the same weights as the original model for fair comparison
    ...
    # Initialize model with simple concatenation fusion
    model = SimpleConcatDamageClassifier(in_channels=3, out_channels=num_classes).to(device)
    
    # Loss components - using the same as the original model for fair comparison
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_iou > best_iou:
            ...
            visualize_simpleconcat_predictions(model, full_dataset, device, num_samples=4, save_path=vis_save_path)
        ...
        if best_model_path:
            ...
            plt.title('Per-Class Accuracy (Simple Concat)')
            plt.ylim([0, 1.0])
