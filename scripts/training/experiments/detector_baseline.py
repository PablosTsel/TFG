#!/usr/bin/env python3
# Baseline Building Detection Model - Simple U-Net with BCE Loss only
# This version uses only BCE loss to establish a baseline for comparison
...
def main():
    """
    Main function to run the baseline binary building segmentation training pipeline.
    Using only BCE loss as a simpler baseline.
    """
    ...
    experiment_name = "baseline_bce_only"
    ...
    # Save configuration
    config = {
        "experiment": "Baseline - BCE Loss Only",
        "timestamp": timestamp,
    ...
    # BASELINE: Use only BCE loss (no Dice loss component)
    ...
    for epoch in range(num_epochs):
        ...
        for images, masks in train_pbar:
            ...
            # Compute BCE loss only (baseline)
